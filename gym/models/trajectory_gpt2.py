...
class Attention(nn.Module):
    def __init__(self, nx, n_ctx, config, scale=False, is_cross_attention=False, attn_index=-1):
        super().__init__()
...
class Block(nn.Module):
    def __init__(self, n_ctx, config, scale=False, attn_index=-1):
        super().__init__()
...
class GPT2Model(GPT2PreTrainedModel):
    def __init__(self, config):
        ...
        self.h = nn.ModuleList([Block(1024, config, scale=True, attn_index=index) for index in range(config.n_layer)])
